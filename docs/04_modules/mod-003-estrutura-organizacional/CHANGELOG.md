...
|--------|------|-------------|-----------|
| 1.15.0 | 2026-04-01 | merge-amendment | Merge FR-002-C01: fix response shape lista departments (pagination→flat conforme paginatedResponse). Route registration, DrizzleDepartmentRepository, DI wiring implementados. Base FR-002 bumped v1.0.1. |
| 1.14.9 | 2026-04-01 | validate-all | Implementação FR-002-C01: (1) departmentsRoutes registrado em src/index.ts, (2) response shape lista corrigido (flat), (3) DrizzleDepartmentRepository criado, (4) 6 use cases + repo registrados no DI. TS: 0 erros. |
| 1.14.8 | 2026-04-01 | create-amendment | Amendment FR-002-C01: fix departmentsRoutes não registrado em src/index.ts (rotas inacessíveis) + response shape da lista departments (pagination wrapper → flat conforme paginatedResponse Zod) + DrizzleDepartmentRepository + DI wiring. Detectado por /validate-all. |
| 1.14.7 | 2026-04-01 | merge-amendment | Merge UX-001-C05: fix checkbox "Mostrar inativos" — seção "Data Wiring" adicionada ao UX-001, nota include_inactive na tabela de ações, soft-deleted nodes expandido. Base UX-001 bumped v0.4.4. |
| 1.14.6 | 2026-04-01 | merge-amendment | Merge FR-001-C05: fix response schema → genericMessageResponse Zod. FR-002 tree query aceita ?include_inactive=true. 4 cenários Gherkin adicionados. Base FR-001 bumped v0.4.1. |
| 1.14.5 | 2026-04-01 | create-amendment | Amendment UX-001-C05: fix checkbox "Mostrar inativos" sem efeito — conectar fetchOrgTree, useOrgTree e OrgTreePage ao query param include_inactive do backend. Ref: spec-fix-org-unit-deactivate-serialization-and-inactive-filter. |
| 1.14.4 | 2026-04-01 | create-amendment | Amendment FR-001-C05: fix response schema JSON literal em DELETE e restore (safeParse error) + adicionar query param include_inactive ao GET /tree. Ref: spec-fix-org-unit-deactivate-serialization-and-inactive-filter. |
