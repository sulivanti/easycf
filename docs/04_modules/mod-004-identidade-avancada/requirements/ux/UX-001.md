...
> | 0.4.0  | 2026-03-31 | merge-amendment | Merge UX-001-M01: layout Penpot 40-Identity. OrgScopePage cards→tabela full-width, ScopePill, TreeSelector N1-N4, TabBar underline, Banner, Drawers 480px, Modais warning, autocomplete usuarios, estados skeleton/empty/error refinados, rotas /organizacao/identidade/* |
...
1. Admin acessa `/organizacao/identidade/escopos`
2. Skeleton exibido durante GET `/admin/org-scopes` (listagem agrupada)
3. Tabela full-width exibida com avatar+nome+email, ScopePill PRIMARY (azul) / SECONDARY (cinza), escopos adicionais com pills + "+N" overflow
...
## Layout Visual — Alinhamento Penpot 40-Identity (UX-001-M01)

> **Referência de design:** Spec `40-identity-spec.md`, Ref `40-identity-ref.html`, Frames Penpot: `40-Identity-Scopes`, `40-Identity-Shares`, `40-Identity-ScopeDrawer`, `40-Identity-DelegationDrawer`

### Rotas

| View | Rota anterior | Rota nova |
|---|---|---|
| UX-IDN-001 | `/usuarios/:id/escopo-organizacional` | `/organizacao/identidade/escopos` |
| UX-IDN-002 | `/identidade/compartilhamentos` | `/organizacao/identidade/compartilhamentos` |

### UX-IDN-001 — Layout Tabela Full-Width

A view de Escopo Organizacional adota layout de tabela em vez de cards:

- **PageHeader:** título "Escopo Organizacional" (24px 800 #111) + subtítulo + botão "+ Atribuir Escopo" (primary #2E86C1)
- **ToolBar:** SearchBar (320×40, busca por nome/email) + FilterTipo (PRIMARY/SECONDARY) + FilterStatus (Ativos/Expirados)
- **OrgScopeTable:** tabela full-width (fill #FFF, r:12) com colunas USUÁRIO (avatar 32×32 + nome + email), ESCOPO PRINCIPAL (ScopePill), ESCOPOS ADICIONAIS (pills + "+N"), STATUS (badge), AÇÕES (pencil + x-circle)
- **ScopePill:** componente com variantes PRIMARY (bg #E3F2FD, text #2E86C1, badge #2E86C1), SECONDARY (bg #F5F5F3, text #555), OVERFLOW ("+N")
- **OrgScopeDrawer:** 480px slide-in com Autocomplete de usuário (GET /users?q=), RadioGroup PRIMARY/SECONDARY, TreeSelector N1-N4 com checkboxes, DatePicker
- **TreeSelector:** componente reutilizável (shared/ui) com checkboxes hierárquicos, indent 24px/nível, N1-N4 apenas, sem inativos
- **RemoveScopeModal:** 420px com ícone warning circular, variantes por tipo (PRIMARY: bg #FFEBEE, SECONDARY: padrão)
- **Endpoint novo:** GET /admin/org-scopes (`admin_org_scopes_list`) com busca textual, filtros, paginação cursor (FR-001-M01)
- **Skeleton:** 6 SkeletonRows com circle + rects pulse
- **Empty:** ícone org-tree 120×120 + "Nenhum escopo atribuído." + BtnAtribuir

### UX-IDN-002 — TabBar Underline e Componentes Reestruturados

A view de Compartilhamentos e Delegações adota TabBar com underline e seções refinadas:

- **PageHeader:** título + subtítulo "Gerencie compartilhamentos de acesso e delegações de escopo" + botão dinâmico (label muda conforme aba)
- **TabBar:** componente reutilizável (shared/ui) com border-bottom 2px, tabs "Meus Compartilhamentos" / "Minhas Delegações" / "Acessos Recebidos", navegação por teclado (Arrow keys)
- **Tab Compartilhamentos:** tabela com colunas COMPARTILHADO COM (avatar 28×28 + nome), ESCOPO/RECURSO, TIPO, VALIDADE, STATUS, AÇÕES. Grantee exibido com avatar+nome via respostas expandidas (FR-001-M01)
- **Tab Delegações:** seções "Delegações Dadas" e "Delegações Recebidas" com SectionTitle 16px 700. Banner re-delegação ANTES da tabela. Colunas: DELEGADO, ESCOPOS (pills), PERÍODO, MOTIVO, AÇÕES
- **Tab Recebidos:** Banner "acessos temporários", seções "Compartilhamentos Recebidos" e "Delegações Recebidas" com Banner re-delegação
- **Banner:** componente reutilizável (shared/ui) com variantes INFO (bg #E3F2FD, text #2E86C1) e WARNING (bg #FFF3E0, text #E67E22), role="alert"
- **CreateShareDrawer:** 480px com Autocomplete grantee (GET /users?q=), Select resource_type, Autocomplete resource_id, MultiSelect allowed_actions, Textarea motivo, Autocomplete autorizador com badges condicionais
- **CreateDelegationDrawer:** 480px com Banner re-delegação no topo, Autocomplete delegatee, MultiSelect escopos com :approve/:execute/:sign DESABILITADOS (aria-disabled + tooltip), DateRangePicker início+fim, Textarea motivo, botão "Criar Delegação"
- **RevokeModal:** 420px com ícone warning circular (48×48 bg #FFF3E0), variantes "Revogar compartilhamento?" / "Revogar delegação?", foco no Cancelar por padrão
- **Empty states:** com ícones contextuais (share 80×80, delegation 80×80, inbox 80×80) + mensagem + BtnCriar

### Componentes Reutilizáveis Novos

| Componente | Path sugerido | Descrição |
|---|---|---|
| ScopePill | `identity-advanced/components/ScopePill.tsx` | Pill de escopo org com badge PRIMARY/SECONDARY/OVERFLOW |
| TreeSelector | `shared/ui/TreeSelector.tsx` | Seletor de árvore org com checkboxes N1-N4 |
| TabBar | `shared/ui/TabBar.tsx` | Barra de abas com underline, teclado, aria |
| Banner | `shared/ui/Banner.tsx` | Banner info/warning com role="alert" |
...
- **rastreia_para:** US-MOD-004, US-MOD-004-F03, US-MOD-004-F04, FR-001, FR-001-M01, BR-001, BR-001.4, BR-001.7, BR-001.8, BR-001.9, BR-001.10, BR-001.11, BR-001.12, SEC-001, SEC-002, DATA-003, DOC-UX-010, DOC-ARC-003, 40-identity-spec.md, 40-identity-ref.html
