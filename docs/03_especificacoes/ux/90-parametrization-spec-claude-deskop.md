# 90-Parametrization — Spec Definitiva v2
...
> **Evolução v2:** Matriz de Incidência tri-state (OBR / OPC / AUTO) substitui checkbox binário
...
| Topbar direita | "Admin ECF" (12px 700 #111) / "A1 Engenharia" (10px #888) + avatar "AE" (40×40) |
...
| **Matriz de Incidência** | **Tri-state por célula** (não checkbox binário) |
...
── MATRIZ TRI-STATE (novo v2) ──
CELL EMPTY           bg:#F5F5F3  border:1.5px #E8E8E6  (estado vazio)
CELL OBR             bg:#EDE7F6  border:1.5px #B39DDB  text:#4A148C  (obrigatório — roxo)
CELL OPC             bg:#E0F2F1  border:1.5px #80CBC4  text:#004D40  (opcional — teal)
CELL AUTO            bg:#FFF8E1  border:1.5px #FFD54F  text:#E65100  (auto-apply — âmbar)
MATRIX ROW ACTIVE    dot 6×6 #27AE60  (indicador: enquadrador com ≥1 regra)
MATRIX HEADER BG     #FAFAFA     Header da matriz
MATRIX LEGEND        text:12px #888  swatches:12×12 r:3
...
MATRIX (v2 — tri-state)
  ...
  Cell label "OBR"                700   9px  uppercase  ls:+0.3px  (cor da ramp)
  Cell label "OPC"                700   9px  uppercase  ls:+0.3px  (cor da ramp)
  Cell label "AUTO"               700   9px  uppercase  ls:+0.3px  (cor da ramp)
  Legend label                    400  12px  #888888
  Legend swatch                   —    12×12 r:3
...
└── ContentArea (1200×836, fill #F5F5F3, padding:24px)
    ...
    │           │
    │           ├── MatrixGrid (display grid, grid-template-columns: 140px repeat(4, 1fr))
    │           │   ├── HeaderRow (bg #FAFAFA, border-bottom 1px #F0F0EE)
    │           │   │   ├── Cell (vazio, w:140px)
    │           │   │   ├── ColHeader "Rot. Validação" (10px 700 uppercase #888, text-align center)
    ...
    │           │   │   ├── RowLabel (flex, gap:8px, align-center)
    │           │   │   │   ├── ActiveDot (6×6, r:50%, fill #27AE60)
    │           │   │   │   └── "Enq. Compras" (12px 600 #333)
    │           │   │   ├── TriStateCell OBR (pad 38×32, r:6, bg #EDE7F6, border 1.5px #B39DDB)
    │           │   │   │   └── "OBR" (9px 700 #4A148C)
    │           │   │   ├── TriStateCell OPC (pad 38×32, r:6, bg #E0F2F1, border 1.5px #80CBC4)
    │           │   │   │   └── "OPC" (9px 700 #004D40)
    │           │   │   ├── TriStateCell EMPTY (pad 38×32, r:6, bg #F5F5F3, border 1.5px #E8E8E6)
    │           │   │   └── TriStateCell OBR
    │           │   │
    │           │   ├── DataRow (selected row, bg #E3F2FD)
    │           │   │   ├── RowLabel
    │           │   │   │   ├── ActiveDot (#27AE60)
    │           │   │   │   └── "Enq. RH" (12px 600 #2E86C1)
    │           │   │   ├── TriStateCell OBR
    │           │   │   ├── TriStateCell EMPTY
    │           │   │   ├── TriStateCell AUTO (pad 38×32, r:6, bg #FFF8E1, border 1.5px #FFD54F)
    │           │   │   │   └── "AUTO" (9px 700 #E65100)
    │           │   │   └── TriStateCell OPC
    │           │   │
    │           │   └── DataRow
    ...
    │               ├── LegendItem: Swatch (12×12, r:3, bg #EDE7F6, border #B39DDB) + "Obrigatório" (12px 400 #888)
    │               ├── LegendItem: Swatch (bg #E0F2F1, border #80CBC4) + "Opcional" (12px 400 #888)
    │               └── LegendItem: Swatch (bg #FFF8E1, border #FFD54F) + "Auto-apply" (12px 400 #888)
```
...
└── ContentArea (1200×836, fill #F5F5F3, padding:24px)
    ...
    └── SplitView (flex, gap:16px, flex:1)
        ...
        └── RightPanel (flex:1, fill #FFF, r:12, border #E8E8E6, flex-col)
            ...
            │   │   ├── TypeBadge "DEFAULT"
            │   │   ├── Ordinal "#4"
            │   │   └── Description "Valor padrão para centro de custo = CC-100"
            │   │
            │   └── RoutineItem
            │       ├── TypeBadge "EVIDENCE"
            │       ├── Ordinal "#5"
            │       └── Description "Exigir nota fiscal quando valor > R$ 5.000"
            │
...
└── ModalCard (w:560, r:12, fill #FFF, shadow 0 8px 32px rgba(0,0,0,0.12), p:24px)
    ...
    │   ├── RuleResult OBR (p:8px 12px, r:6, bg #EDE7F6, mb:8px)
    │   │   ├── RuleName "Rot. Validação → VALIDATION" (12px 600 #4A148C)
    │   │   └── RuleDetail "Valor máximo R$ 50.000 — BLOQUEANTE" (11px 400 #333, mt:2px)
    │   ├── RuleResult OBR (p:8px 12px, r:6, bg #EDE7F6, mb:8px)
    │   │   ├── RuleName "Rot. Validação → REQUIRED" (12px 600 #4A148C)
    │   │   └── RuleDetail "Campo valor obrigatório quando tipo=Compra"
    │   └── RuleResult OPC (p:8px 12px, r:6, bg #E0F2F1)
    │       ├── RuleName "Rot. Visibilidade → FIELD_VISIBILITY" (12px 600 #004D40)
    │       └── RuleDetail "Ocultar campo CNPJ quando tipo=PF"
    │
...
├── PanelHeader (normal)
└── SkeletonGrid (3×4)
    └── Rect 38×32 r:6 bg:#E8E8E6 animate pulse  (tri-state cell placeholder)
```
...
Matrix cell           auto×48     border-bottom:1px #F0F0EE  (grid cell container)
TriState pad          38×32       r:6  border:1.5px (cor por estado)
TriState pad empty    38×32       r:6  bg:#F5F5F3  border:1.5px #E8E8E6
TriState pad OBR      38×32       r:6  bg:#EDE7F6  border:1.5px #B39DDB
TriState pad OPC      38×32       r:6  bg:#E0F2F1  border:1.5px #80CBC4
TriState pad AUTO     38×32       r:6  bg:#FFF8E1  border:1.5px #FFD54F
Matrix legend swatch  12×12       r:3
Active dot            6×6         r:50% fill:#27AE60
...
| `parametrization/IncidenceMatrix` | **Grid tri-state** enquadrador × rotina com 3 estados (OBR/OPC/AUTO) + legenda + filtro PUBLISHED | RightPanel (View 1) |
| `parametrization/TriStateCell` | **Célula clicável** que cicla entre vazio → OBR → OPC → AUTO, com pad colorido e label | IncidenceMatrix |
| `parametrization/MatrixLegend` | Legenda inline com 3 swatches + labels | IncidenceMatrix |
| `parametrization/DryRunModal` | Modal de simulação dry-run com select e resultados color-coded por tipo de incidência | ParametrizationPage |
...
- [ ] Breadcrumb: "Processos › Parametrização"
- [ ] Topbar direita: Variante C (tb-un + tb-ue + avatar 40×40)
...
- [ ] **Right panel: matriz tri-state** enquadrador × rotina com pads coloridos (OBR roxo / OPC teal / AUTO âmbar)
- [ ] **Tri-state cells** 38×32 r:6, label 9px 700 uppercase, border 1.5px
- [ ] **Legenda da matriz** com 3 swatches (Obrigatório / Opcional / Auto-apply) abaixo do grid
- [ ] **Active dots** 6×6 verde ao lado do nome de cada enquadrador com ≥1 regra
- [ ] Filtro "Somente PUBLISHED" na matriz de incidência
- [ ] Modal dry-run com aviso de não persistência + select de objetos + resultados **color-coded por tipo** (OBR roxo / OPC teal)
...
- [ ] 5 itens visíveis no editor (v3 · 5 itens)
...
- [ ] Loading skeleton nos 3 painéis (tri-state cells como rect 38×32)
