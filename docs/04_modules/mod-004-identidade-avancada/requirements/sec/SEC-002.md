...
> | 0.3.0  | 2026-03-17 | AGN-DEV-06  | Enriquecimento Batch 3 — alinhamento BR-001.10-12, validação integridade referencial |

# SEC-002 — Matriz de Autorização de Eventos da Identidade Avançada
...
## Validações de Integridade nos Comandos Emissores (MUST)

As ações que emitem domain events DEVEM validar integridade referencial antes da emissão, garantindo que eventos inválidos não contaminem a timeline:

| Validação | Comandos afetados | Regra | Ref |
|---|---|---|---|
| `valid_until` futuro | Todos os POST (shares, delegations, org_scopes com vigência) | Rejeitar 422 antes de emit | BR-001.10 |
| `org_unit_id` ativo em MOD-003 | POST org_scopes | Rejeitar 422 se nó inexistente/inativo | BR-001.11 |
| `grantee_id`/`delegatee_id` no tenant | POST shares, POST delegations | Rejeitar 422 se usuário fora do tenant | BR-001.12 |
| Escopos proibidos em delegação | POST delegations | Rejeitar 422 se `:approve`/`:execute`/`:sign` | BR-001.4 |

> **Princípio:** Nenhum domain event deve ser emitido para uma operação que falhou na validação. O Outbox INSERT ocorre na mesma transação do comando, após todas as validações passarem.

---

## Critérios de aceite (Gherkin) — SEC-002
...
- **rastreia_para:** US-MOD-004, US-MOD-004-F01, US-MOD-004-F02, DATA-003, SEC-001, BR-001, BR-001.4, BR-001.10, BR-001.11, BR-001.12, DOC-ARC-003, DOC-FND-000
- **referencias_exemplos:** EX-AUTH-001 (RBAC), EX-PII-001 (mascaramento), EX-SEC-002 (integridade comandos emissores)
- **evidencias:** N/A
