...
> | 0.7.0  | 2026-03-31 | merge-amendment | Merge UX-000-M04: design visual definitivo 07-RoleForm em UX-006 — FormCard 720px, ScopeChips, Toggle status, InfoBox, BackLink. Dois modos (Editar/Criar) com rotas dedicadas. Ref: 07-role-form-spec, PEN-07-RoleForm-Edit, PEN-07-RoleForm-Create. |
> | 0.6.0  | 2026-03-30 | merge-amendment | Merge UX-000-M03: nova jornada UX-010 (Sessão Mantida por Atividade / Keep-Alive Transparente). Ref: spec-smart-session-keepalive, FR-000-M03. |
...
1. Admin acessa "Perfis e Permissões" no menu
2. Lista paginada de roles com filtros e busca
3. Criar: FormCard 720px (branco, r:12, border #E8E8E6, p:32px) com campos:
   - **NOME** — input text 656×48 r:10
   - **DESCRIÇÃO** — textarea 656×80 min-height, r:10, line-height:1.5
   - **Seção Escopos e Permissões** — ScopeGrid (flex-wrap, gap:8px) com ScopeChips removíveis
   - **AddScopeRow** — input (flex:1, 534×42, r:10, placeholder "domínio:entidade:ação") + Button "Adicionar" (110×42)
   - **InfoBox** — #F0F8FF r:8, texto "Os escopos definem quais ações este perfil pode executar..." (#2E86C1)
   - Botões: "Cancelar" (h:44, r:8, border #E8E8E6, text 13px/600/#555555) + "Criar Perfil" (h:44, r:8, fill #2E86C1, text 13px/700/#FFFFFF)
4. Editar: mesmo FormCard, diferenças:
   - Título "Editar Perfil" + StatusBadge ATIVO (10px/700/#1E7A42, bg:#E8F8EF, border #B5E8C9, r:11)
   - Breadcrumb: "Administração › Perfis e Permissões › Editar"
   - Campos preenchidos com valores reais
   - **STATUS** — toggle (track 40×22 r:11 #2E86C1, thumb 18×18 #FFF) + "Ativo"
   - Scope chips preenchidos com scopes do role
   - Botão primary: "Salvar Alterações" (160×44)
5. Excluir: modal de confirmação → soft delete

> **Ref visual:** `07-role-form-spec.md`, `07-role-form-ref.html`, Penpot pages 07-RoleForm-Edit (32/32 conformes, 0 discrepâncias), 07-RoleForm-Create

### Layout Visual — Modo Edição (07-RoleForm-Edit)

```
ContentArea (x=240, y=64, 1200×836, fill #F5F5F3, padding 32px, overflow-y:auto)
├── PageHeader
│   ├── BackLink "← Voltar para lista" (13px/600/#2E86C1, mb:16)
│   ├── TitleRow: Title "Editar Perfil" (28px/800/#111111, ls:-1px) + StatusBadge ATIVO
...
└── FormCard (720×auto, fill #FFF, r:12, border 1px #E8E8E6, padding 32px)
    ...
    │   ├── FormField "NOME" (label 11px/700/#333333, uppercase, ls:0.8px) → input 656×48 r:10
    │   ├── FormField "DESCRIÇÃO" (mt:20) → textarea 656×80 r:10
    │   └── ToggleStatus (mt:20, apenas edição): Label "Status" + Track 40×22 + Thumb 18×18
    ├── Separator (656×1, fill #E8E8E6, mt:28, mb:28)
    ├── Section: Escopos e Permissões
    │   ├── SectionHeader: "ESCOPOS E PERMISSÕES" (11px/700/#888888) + "24 escopos atribuídos" (12px/500/#888888)
    │   ├── ScopeGrid (flex-wrap, gap:8px): ScopeChip ×N (bg #F4F4F2, border #E0E0DE, r:6, p:6px 12px)
    │   ├── AddScopeRow (mt:16): Input (534×42) + "Adicionar" (110×42)
    │   └── InfoBox (mt:16, fill #F0F8FF, r:8, icon 16×16 + text 13px #2E86C1)
    ├── Separator (656×1, fill #E8E8E6, mt:28, mb:28)
    └── FormFooter (flex, gap:12px): "Salvar Alterações" (160×44 primary) + "Cancelar" (100×44 outline)
```

### Dois modos (Frames)

| Item | Edição (`/perfis/:id`) | Criação (`/perfis/novo`) |
|------|----------------------|------------------------|
| Breadcrumb | "Administração › Perfis e Permissões › Editar" | "... › Novo" |
...
| Campos | Preenchidos | Vazios (placeholder only) |
...
### Componentes a implementar (UX-000-M04)

| Componente | Tipo | Localização | Descrição |
|---|---|---|---|
| BackLink | Primitivo | `@shared/ui/back-link.tsx` | "← Voltar para lista", 13px/600, color #2E86C1, hover underline |
| ScopeChip | Primitivo | `foundation/ui/scope-chip.tsx` | Chip label + ×, bg #F4F4F2, border #E0E0DE, r:6 |
| ScopeGrid | Composto | `foundation/ui/scope-grid.tsx` | flex-wrap container de ScopeChips, gap:8px |
| AddScopeRow | Composto | `foundation/ui/add-scope-row.tsx` | Input + Button "Adicionar" em row, gap:12px |
| PageHeaderWithBadge | Composto | `foundation/ui/page-header-badge.tsx` | PageHeader + StatusBadge inline ao título |
| InfoBox | Primitivo | `@shared/ui/info-box.tsx` | Box info com ícone + texto, bg #F0F8FF, r:8 |
| RoleFormPage | Page | `foundation/pages/roles/RoleFormPage.tsx` | Page component para `/perfis/:id` e `/perfis/novo` |

### Estados

- **Loading:** Skeleton nos campos (modo edição, enquanto carrega dados existentes)
- **Empty:** N/A (formulário) / EmptyState "Nenhum escopo adicionado ainda" (scopes em criação)
- **Error:** Inline por campo (422) + Toast RFC 9457 com correlationId (5xx)
...
| `update` | Editar escopos/status | `PUT /api/v1/roles/:id` | `role.updated`, `role.status_changed` |
...
## UX-010 — Sessão Mantida por Atividade (Keep-Alive Transparente) (UX-000-M03)

- **Screen ID:** N/A (comportamento transversal, sem tela dedicada)
- **Entidade(s):** `session`
- **Contexto:** Mecanismo transparente de manutenção de sessão — o sistema renova o token automaticamente enquanto o usuário estiver ativo, sem interrupção da experiência. Complementa UX-001 (cenário reativo de sessão expirada).

### Jornada (Happy Path — Uso Contínuo)
...
### Jornada Alternativa — Retorno após inatividade curta (< 30min)
...
### Jornada Alternativa — Retorno após inatividade longa (> 30min, refresh token válido)
...
### Jornada Alternativa — Sessão expirada (refresh token expirado ou revogado)
...
4. Toast "Sessão expirada. Faça login novamente." na tela de login (complemento UX-000-C01)
...
- **UX-001 (sessão expirada por timeout):** Trata o cenário reativo (401 já aconteceu → redirect login, toast)
- **UX-010 (esta jornada):** Adiciona o cenário proativo (evitar que o 401 aconteça enquanto o usuário está ativo)
...
> **Ref:** UX-000-M03. spec-smart-session-keepalive. FR-023 (FR-000-M03).
