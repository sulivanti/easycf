---
name: validate-fastify-endpoint
...
> **Sobre o campo `type`:** O valor `https://api.projeto.local/errors/{tipo-do-erro}` é um placeholder ilustrativo. Em produção, substitua pelo domínio real da API e pelo slug do tipo de erro específico (ex: `https://api.suaempresa.com/errors/invalid-payload`, `https://api.suaempresa.com/errors/resource-not-found`). O campo deve ser uma URI estável e documentada, não um path dinâmico.

### 2.4 Declaração de Schema OpenAPI na Rota (DOC-ARC-001)

O normativo `DOC-ARC-001` exige que cada rota declare seu contrato diretamente no objeto de opções do Fastify via `schema:`, habilitando validação automática (ajv) e geração de documentação OpenAPI.

- **Regra:** Toda rota não-trivial DEVE declarar `schema: { body?, querystring?, params?, response }` no segundo argumento do `fastify.method(path, { schema, preHandler }, handler)`.
- **Regra:** O schema de `response` DEVE declarar ao menos o código de sucesso principal (ex: `200`, `201`) e o shape de erro padrão (`4xx`/`5xx`) referenciando o modelo `ProblemDetails`.
- **Verificar:** Se a rota não possui declaração `schema:`, marcar como **Violação Moderada** e recomendar adição para compatibilidade com o lint Spectral (`openapi:lint`).
- **Exceção:** Webhooks de terceiros e rotas de health check (`/health`, `/readyz`) podem omitir `schema:` com justificativa em comentário no código.
...
2. **Leitura:** Use sua ferramenta de leitura de arquivo para absorver o conteúdo textual completo do arquivo alvo.
3. **Inspeção a Frio:**
