"""
Create 80-Case-Execution screens in Penpot Sandbox.
Uses transit+json REST API. Based on 80-case-execution-spec-claude-desktop.md.

Pages:
  80-CaseList              — Listagem de casos com tabela e filtros
  80-CasePanel-Gates       — Painel do caso, aba Gates
  80-CasePanel-Assignments — Painel do caso, aba Atribuições
  80-CasePanel-Timeline    — Painel do caso, aba Timeline
  80-CaseList-Drawer       — Listagem com drawer "Novo Caso"
  80-CasePanel-CancelModal — Modal "Cancelar Caso"
  80-CasePanel-WaiveModal  — Modal "Dispensar Gate"
"""
import json
import uuid
import requests
...
# ═══════════════════════════════════════════
# SHARED BUILDERS (Topbar, Sidebar, etc.)
# ═══════════════════════════════════════════

def build_topbar(frame_id, page_id, breadcrumb_items):
    """Build topbar and return list of (id, parent, frame, obj) tuples."""
    ops = []
    tb_id = uid()
    ops.append((tb_id, ROOT, frame_id,
        make_rect(tb_id, "Topbar", 0, 0, 1440, 64, WHITE, ROOT, frame_id,
...
def build_sidebar(frame_id, page_id, active_item="Casos"):
    """Build sidebar and return list of (id, parent, frame, obj) tuples."""
    ops = []
    sb_id = uid()
    ops.append((sb_id, ROOT, frame_id,
        make_rect(sb_id, "Sidebar", 0, 64, 240, 836, WHITE, ROOT, frame_id,
                  strokes=stroke_border(BORDER, 1, "inner"))))
...
def build_content_area(frame_id, page_id):
    """Content area bg rect."""
    ca_id = uid()
    obj = make_rect(ca_id, "ContentArea", 240, 64, 1200, 836, BG_PAGE, ROOT, frame_id)
    return ca_id, [(ca_id, ROOT, frame_id, obj)]
...
def build_case_list(page_id):
    ...
    # Topbar
    ops.extend(build_topbar(frame_id, page_id,
        [("Processos", False), ("Casos", True)]))

    # Sidebar
    ops.extend(build_sidebar(frame_id, page_id))

    # Content area
    ca_id, ca_ops = build_content_area(frame_id, page_id)
    ops.extend(ca_ops)
    ...
    # Search bar
    sb_id = uid()
    ops.append((sb_id, ca_id, frame_id,
        make_rect(sb_id, "SearchBar", 1228, fy, 188, 40, WHITE, ca_id, frame_id, radius=8,
                  strokes=stroke_border(BORDER))))
    sb_t = uid()
    ops.append((sb_t, sb_id, frame_id,
        make_text(sb_t, "SearchText", 1264, fy+12, 140, 16, "Buscar por código...", 12, 400, T6, sb_id, frame_id)))
...
# ═══════════════════════════════════════════
# SHARED: Case Panel Header (HeaderCard + Progress + Tabs)
# ═══════════════════════════════════════════
...
def build_case_header(frame_id, ca_id, active_tab="Gates"):
    ...
    py = 192
    seg_w = 380
    colors = [SUCCESS, BLUE, BORDER]
    labels = ["Abertura", "Revisão", "Aprovação N2"]
...
def build_case_panel_gates(page_id):
    ...
    ops.extend(build_topbar(frame_id, page_id,
        [("Processos", False), ("Casos", False), ("CASO-2026-0042", True)]))
    ops.extend(build_sidebar(frame_id, page_id))
    ...
    for gi, (gname, gtype, gstatus, gkey, has_actions) in enumerate(gates):
        ...
        if gname == "Checklist de Conformidade":
            gc_h = 160  # extra space for checklist
        ...
        if gname == "Checklist de Conformidade":
            # Checklist items
            checks = [
                ("Formulário preenchido", True),
                ("Documentos digitalizados", True),
                ("Assinaturas coletadas", True),
                ("Validação técnica", False),
                ("Aprovação financeira", False),
            ]
            cy = gy + 60
            for ci, (ctext, checked) in enumerate(checks):
                ...
                cl = uid()
                color_c = T1 if checked else T4
                ops.append((cl, gc_id, frame_id,
                    make_text(cl, f"CheckLabel-{ci}", 308, cy, 300, 16, ctext, 13, 500, color_c, gc_id, frame_id)))
                cy += 24
...
def build_case_panel_assignments(page_id):
    ...
    for ai, (role, person, initials, unassigned) in enumerate(assignments):
        ac_h = 80 if person else 76
        ac_id = uid()
        strokes = stroke_border(BORDER)
        if unassigned:
            # Left red border - we'll add it as extra stroke
            strokes = [["^ ",
                "~:stroke-style", "~:solid",
...
def build_case_list_drawer(page_id):
    ...
    # Dimmed content (simplified table placeholder)
    dim = uid()
    ...
    # Drawer overlay
    ov_id = uid()
    ops.append((ov_id, ca_id, frame_id,
        make_rect(ov_id, "DrawerOverlay", 960, 0, 480, 900, WHITE, ca_id, frame_id,
                  strokes=[["^ ",
                      "~:stroke-style", "~:solid",
    ...
    for fi, (label, ftype, placeholder, fh) in enumerate(fields):
        ...
        if ftype == "select" or ftype == "textarea":
            ff = uid()
...
def build_cancel_modal(page_id):
    ...
    # Dimmed overlay
    ov = uid()
    ops.append((ov, ca_id, frame_id,
        make_rect(ov, "ModalOverlay", 0, 0, 1440, 900, T1, ca_id, frame_id, opacity=0.3)))

    # Modal card
    mc_id = uid()
    ops.append((mc_id, ca_id, frame_id,
        make_rect(mc_id, "ModalCard", 510, 300, 420, 220, WHITE, ca_id, frame_id, radius=12)))
    ...
    ops.append((mm, mc_id, frame_id,
        make_text(mm, "ModalMsg", 540, 412, 360, 32, "Deseja cancelar o caso CASO-2026-0042? Esta ação não pode ser desfeita.", 13, 400, T3, mc_id, frame_id)))
...
def build_waive_modal(page_id):
    ...
    mc_id = uid()
    ops.append((mc_id, ca_id, frame_id,
        make_rect(mc_id, "ModalCard", 510, 260, 420, 310, WHITE, ca_id, frame_id, radius=12)))
    ...
    mm = uid()
    ops.append((mm, mc_id, frame_id,
        make_text(mm, "ModalMsg", 540, 372, 360, 32, "Deseja dispensar o gate Aprovação do Gestor? Informe o motivo abaixo.", 13, 400, T3, mc_id, frame_id)))
    ...
    ops.append((mp, mf, frame_id,
        make_text(mp, "MotivoPlaceholder", 546, 446, 340, 14, "Descreva o motivo da dispensa (mín. 20 caracteres)...", 13, 400, T6, mf, frame_id)))
    mc = uid()
...
# ═══════════════════════════════════════════
# MAIN: Create all pages and objects
# ═══════════════════════════════════════════
...
for page_name, builder in pages:
    ...
    if not send_changes(page_changes, f"add-page {page_name}"):
        print(f"FAILED to create page {page_name}, skipping")
        continue

    # Step 2: Build all objects
    frame_id, all_ops = builder(page_id)

    # Step 3: Send objects in batches of 40
    changes = []
    ...
    for i in range(0, len(changes), batch_size):
        ...
        batch_label = f"{page_name} objects [{i+1}-{min(i+batch_size, len(changes))}]"
        if not send_changes(batch, batch_label):
            print(f"FAILED batch {batch_label}")
            break

    total_objects += len(all_ops)
    print(f"  Objects created: {len(all_ops)}")

print(f"\n{'='*50}")
print(f"DONE — {len(pages)} pages, {total_objects} objects total")
