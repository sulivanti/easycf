...
export class DrizzleFramerTypeRepository implements FramerTypeRepository {
  ...
  async findById(tenantId: string, id: string, tx?: TransactionContext): Promise<ContextFramerTypeRecord | null> {
    const c = conn(this.db, tx);
    const [row] = await c.select().from(contextFramerTypes)
      .where(and(eq(contextFramerTypes.id, id), eq(contextFramerTypes.tenantId, tenantId)))
      .limit(1);
    return row ?? null;
  }

  async list(tenantId: string, params: PaginationParams, tx?: TransactionContext): Promise<PaginatedResult<ContextFramerTypeRecord>> {
    const c = conn(this.db, tx);
    const limit = params.limit + 1;
    const rows = await c.select().from(contextFramerTypes)
  ...
  async create(record: ContextFramerTypeRecord, tx?: TransactionContext): Promise<ContextFramerTypeRecord> {
    const c = conn(this.db, tx);
...
export class DrizzleFramerRepository implements FramerRepository {
  ...
  async findById(tenantId: string, id: string, tx?: TransactionContext): Promise<ContextFramerProps | null> {
    const c = conn(this.db, tx);
    const [row] = await c.select().from(contextFramers)
      .where(and(eq(contextFramers.id, id), eq(contextFramers.tenantId, tenantId), isNull(contextFramers.deletedAt)))
      .limit(1);
  ...
  async list(
    ...
    const rows = await c.select().from(contextFramers)
  ...
  async update(record: ContextFramerProps, tx?: TransactionContext): Promise<ContextFramerProps> {
    const c = conn(this.db, tx);
    const [row] = await c.update(contextFramers)
      .set({ nome: record.nome, status: record.status, validFrom: record.validFrom, validUntil: record.validUntil, version: record.version, updatedAt: record.updatedAt })
  ...
  async softDelete(tenantId: string, id: string, tx?: TransactionContext): Promise<void> {
    const c = conn(this.db, tx);
    await c.update(contextFramers)
      .set({ deletedAt: new Date(), updatedAt: new Date() })
      .where(and(eq(contextFramers.id, id), eq(contextFramers.tenantId, tenantId)));
  }

  async findExpired(tenantId: string, now: Date, tx?: TransactionContext): Promise<readonly ContextFramerProps[]> {
    const c = conn(this.db, tx);
    const rows = await c.select().from(contextFramers)
      .where(and(
  ...
  private toDomain(row: any): ContextFramerProps {
    return {
      ...
      framerTypeId: row.framerTypeId, status: row.status, version: row.version,
...
export class DrizzleTargetObjectRepository implements TargetObjectRepository {
  ...
  async findById(tenantId: string, id: string, tx?: TransactionContext): Promise<TargetObjectRecord | null> {
    const c = conn(this.db, tx);
    const [row] = await c.select().from(targetObjects)
      .where(and(eq(targetObjects.id, id), eq(targetObjects.tenantId, tenantId)))
      .limit(1);
    return row ?? null;
  }

  async list(tenantId: string, params: PaginationParams, tx?: TransactionContext): Promise<PaginatedResult<TargetObjectRecord>> {
    const c = conn(this.db, tx);
    const limit = params.limit + 1;
    const rows = await c.select().from(targetObjects)
...
export class DrizzleTargetFieldRepository implements TargetFieldRepository {
  ...
  async findById(tenantId: string, id: string, tx?: TransactionContext): Promise<TargetFieldRecord | null> {
    const c = conn(this.db, tx);
    const [row] = await c.select().from(targetFields)
      .where(and(eq(targetFields.id, id), eq(targetFields.tenantId, tenantId)))
      .limit(1);
    return row ?? null;
  }

  async listByTargetObject(tenantId: string, targetObjectId: string, tx?: TransactionContext): Promise<readonly TargetFieldRecord[]> {
    const c = conn(this.db, tx);
    return c.select().from(targetFields)
      .where(and(eq(targetFields.targetObjectId, targetObjectId), eq(targetFields.tenantId, tenantId)));
  }
...
export class DrizzleIncidenceRuleRepository implements IncidenceRuleRepository {
  ...
  async findById(tenantId: string, id: string, tx?: TransactionContext): Promise<IncidenceRuleProps | null> {
    const c = conn(this.db, tx);
    const [row] = await c.select().from(incidenceRules)
  ...
  async list(
    tenantId: string,
    params: PaginationParams & { framerId?: string; targetObjectId?: string; status?: 'ACTIVE' | 'INACTIVE' },
    tx?: TransactionContext,
    ...
    if (params.targetObjectId) conditions.push(eq(incidenceRules.targetObjectId, params.targetObjectId));
    if (params.status) conditions.push(eq(incidenceRules.status, params.status));

    const rows = await c.select().from(incidenceRules)
      .where(and(...conditions))
  ...
  async findByFramerAndObject(tenantId: string, framerId: string, targetObjectId: string, tx?: TransactionContext): Promise<IncidenceRuleProps | null> {
    const c = conn(this.db, tx);
    const [row] = await c.select().from(incidenceRules)
      .where(and(
  ...
  async countByFramer(tenantId: string, framerId: string, tx?: TransactionContext): Promise<number> {
    const c = conn(this.db, tx);
  ...
  async update(record: IncidenceRuleProps, tx?: TransactionContext): Promise<IncidenceRuleProps> {
    const c = conn(this.db, tx);
    const [row] = await c.update(incidenceRules)
      .set({ conditionExpr: record.conditionExpr, validFrom: record.validFrom, validUntil: record.validUntil, status: record.status, updatedAt: record.updatedAt })
      .where(eq(incidenceRules.id, record.id))
      .returning();
    return this.toDomain(row);
  }

  async findActiveByFramerIds(tenantId: string, framerIds: readonly string[], now: Date, tx?: TransactionContext): Promise<readonly IncidenceRuleProps[]> {
  ...
  private toDomain(row: any): IncidenceRuleProps {
    return {
      id: row.id, tenantId: row.tenantId, framerId: row.framerId, targetObjectId: row.targetObjectId,
      conditionExpr: row.conditionExpr, validFrom: row.validFrom, validUntil: row.validUntil,
...
export class DrizzleRoutineRepository implements RoutineRepository {
  ...
  async findById(tenantId: string, id: string, tx?: TransactionContext): Promise<BehaviorRoutineProps | null> {
    const c = conn(this.db, tx);
    const [row] = await c.select().from(behaviorRoutines)
      .where(and(eq(behaviorRoutines.id, id), eq(behaviorRoutines.tenantId, tenantId), isNull(behaviorRoutines.deletedAt)))
      .limit(1);
    return row ? this.toDomain(row) : null;
  }
  ...
  async list(
    tenantId: string,
    params: PaginationParams & { status?: 'DRAFT' | 'PUBLISHED' | 'DEPRECATED'; routineType?: 'BEHAVIOR' | 'INTEGRATION' },
    tx?: TransactionContext,
  ): Promise<PaginatedResult<BehaviorRoutineProps>> {
    const c = conn(this.db, tx);
    const limit = params.limit + 1;
    const conditions = [eq(behaviorRoutines.tenantId, tenantId), isNull(behaviorRoutines.deletedAt)];
    ...
    const rows = await c.select().from(behaviorRoutines)
      .where(and(...conditions))
  ...
  async create(record: BehaviorRoutineProps, tx?: TransactionContext): Promise<BehaviorRoutineProps> {
    const c = conn(this.db, tx);
    const [row] = await c.insert(behaviorRoutines).values(record).returning();
    return this.toDomain(row);
  }

  async update(record: BehaviorRoutineProps, tx?: TransactionContext): Promise<BehaviorRoutineProps> {
    const c = conn(this.db, tx);
    const [row] = await c.update(behaviorRoutines)
      .set({
        nome: record.nome, status: record.status, version: record.version,
        publishedAt: record.publishedAt, approvedBy: record.approvedBy, updatedAt: record.updatedAt,
      })
  ...
  private toDomain(row: any): BehaviorRoutineProps {
    return {
      id: row.id, tenantId: row.tenantId, codigo: row.codigo, nome: row.nome,
      routineType: row.routineType, version: row.version, status: row.status,
      parentRoutineId: row.parentRoutineId, publishedAt: row.publishedAt, approvedBy: row.approvedBy,
      createdBy: row.createdBy, createdAt: row.createdAt, updatedAt: row.updatedAt, deletedAt: row.deletedAt,
    };
  }
}
...
export class DrizzleRoutineItemRepository implements RoutineItemRepository {
  ...
  async listByRoutine(routineId: string, tx?: TransactionContext): Promise<readonly RoutineItemRecord[]> {
    const c = conn(this.db, tx);
    const rows = await c.select().from(routineItems)
  ...
  async countByRoutine(routineId: string, tx?: TransactionContext): Promise<number> {
    const c = conn(this.db, tx);
    const [result] = await c.select({ count: dsql<number>`count(*)::int` })
      .from(routineItems).where(eq(routineItems.routineId, routineId));
    return result?.count ?? 0;
  }
  ...
  async create(record: RoutineItemRecord, tx?: TransactionContext): Promise<RoutineItemRecord> {
    const c = conn(this.db, tx);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const [row] = await c.insert(routineItems).values(record as any).returning();
    return this.toDomain(row);
  }

  async update(record: RoutineItemRecord, tx?: TransactionContext): Promise<RoutineItemRecord> {
    const c = conn(this.db, tx);
    const [row] = await c.update(routineItems)
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .set({
        itemType: record.itemType as any, targetFieldId: record.targetFieldId, action: record.action as any,
        value: record.value, conditionExpr: record.conditionExpr, validationMessage: record.validationMessage,
        isBlocking: record.isBlocking, ordem: record.ordem, updatedAt: record.updatedAt,
      })
  ...
  async copyToRoutine(sourceRoutineId: string, targetRoutineId: string, tx?: TransactionContext): Promise<readonly RoutineItemRecord[]> {
    const c = conn(this.db, tx);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const rows = await c.insert(routineItems).values(newItems as any).returning();
    return rows.map((r) => this.toDomain(r));
  }
  ...
  private toDomain(row: any): RoutineItemRecord {
    return {
      id: row.id, routineId: row.routineId, itemType: row.itemType,
      targetFieldId: row.targetFieldId, action: row.action, value: row.value,
      conditionExpr: row.conditionExpr, validationMessage: row.validationMessage,
      isBlocking: row.isBlocking, ordem: row.ordem,
...
export class DrizzleRoutineIncidenceLinkRepository implements RoutineIncidenceLinkRepository {
  ...
  async findByRoutineAndRule(routineId: string, incidenceRuleId: string, tx?: TransactionContext): Promise<RoutineIncidenceLinkRecord | null> {
    const c = conn(this.db, tx);
    const [row] = await c.select().from(routineIncidenceLinks)
      .where(and(eq(routineIncidenceLinks.routineId, routineId), eq(routineIncidenceLinks.incidenceRuleId, incidenceRuleId)))
      .limit(1);
    return row ?? null;
  }

  async listByRoutine(routineId: string, tx?: TransactionContext): Promise<readonly RoutineIncidenceLinkRecord[]> {
    const c = conn(this.db, tx);
    return c.select().from(routineIncidenceLinks).where(eq(routineIncidenceLinks.routineId, routineId));
  }

  async findPublishedRoutineIdsByRuleIds(ruleIds: readonly string[], tx?: TransactionContext): Promise<readonly { routineId: string; incidenceRuleId: string }[]> {
    if (ruleIds.length === 0) return [];
    const c = conn(this.db, tx);
    const rows = await c
      .select({ routineId: routineIncidenceLinks.routineId, incidenceRuleId: routineIncidenceLinks.incidenceRuleId })
  ...
  async create(record: RoutineIncidenceLinkRecord, tx?: TransactionContext): Promise<RoutineIncidenceLinkRecord> {
    const c = conn(this.db, tx);
  ...
  async deleteByRoutineAndRule(routineId: string, incidenceRuleId: string, tx?: TransactionContext): Promise<void> {
    const c = conn(this.db, tx);
    await c.delete(routineIncidenceLinks).where(
      and(eq(routineIncidenceLinks.routineId, routineId), eq(routineIncidenceLinks.incidenceRuleId, incidenceRuleId)),
    );
  }

  async copyToRoutine(sourceRoutineId: string, targetRoutineId: string, tx?: TransactionContext): Promise<readonly RoutineIncidenceLinkRecord[]> {
    const c = conn(this.db, tx);
...
export class DrizzleVersionHistoryRepository implements VersionHistoryRepository {
  ...
  async listByRoutine(routineId: string, tx?: TransactionContext): Promise<readonly RoutineVersionHistoryRecord[]> {
    const c = conn(this.db, tx);
    return c.select().from(routineVersionHistory)
      .where(eq(routineVersionHistory.routineId, routineId))
      .orderBy(desc(routineVersionHistory.changedAt));
  }

  async create(record: RoutineVersionHistoryRecord, tx?: TransactionContext): Promise<RoutineVersionHistoryRecord> {
    const c = conn(this.db, tx);
