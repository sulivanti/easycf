...
> |--------|------------|-------------|-------------------|
> | 0.4.4  | 2026-04-01 | merge-amendment | Merge UX-001-C05: fix checkbox "Mostrar inativos" sem efeito — `fetchOrgTree` aceita `includeInactive`, `useOrgTree` propaga na queryKey, `OrgTreePage` passa `showInactive` ao hook. Ref: spec-fix-org-unit-deactivate-serialization-and-inactive-filter. |
> | 0.4.3  | 2026-03-31 | merge-amendment | Merge UX-001-C04: fix "Vincular tenant" em N4 redireciona para edição inline. LinkTenantModal (novo), handler onLinkTenant corrigido, seção "Estabelecimentos Vinculados" renomeada, DetailPanel prop onLinkTenant. Ref: spec-fix-org-n4-link-tenant-edit-redirect. |
...
|---|---|---|---|---|---|---|
| `view` | query | collection | `GET /api/v1/org-units/tree` (aceita `?include_inactive=true` — UX-001-C05) | `org_units_tree` | — | `org:unit:read` |
...
- **Soft-deleted nodes:** Exibidos com opacidade reduzida e badge "Inativo" apenas quando toggle "Mostrar inativos" esta ativo (default: desativado). O toggle DEVE passar `includeInactive=true` ao `useOrgTree()` que envia `?include_inactive=true` ao endpoint tree (UX-001-C05). A filtragem client-side em `OrgTreeNode.visibleChildren` permanece como safety net.
...
### Copy Catalog (LinkTenantModal — UX-001-C04)

| Chave | Mensagem |
|-------|----------|
| `modal.linkTenantTitle` | "Vincular Estabelecimento" |
| `modal.linkTenantSubtitle` | `Selecione um estabelecimento para vincular a '${orgUnitNome}'.` |
...
└── PainelDetalhe (flex, fill #F5F5F3, padding 24px, overflow-y auto)
    ...
    ├── CardEstabelecimentos (card r:12, p:24px, mt:20px) — UX-001-C04
    │   ├── "ESTABELECIMENTOS VINCULADOS" + "Ver todos (N)" link azul
    │   ├── Tags wrap (r:6, border #E8E8E6) — tenants N5 vinculados ao N4
    │   └── "+ Vincular Estabelecimento" (dashed #CCC, visível se nivel===4 && hasWrite && !isEditing)
    │       → abre LinkTenantModal (NÃO modo edição)
...
export interface DetailPanelProps {
  ...
  onLinkTenant: (orgUnitId: string) => void; // UX-001-C04
  isEditing: boolean;
...
**Ações NÃO interceptadas:** scroll na árvore, toggle "Mostrar inativos" (dispara refetch — UX-001-C05), busca na SearchBar, botões "Cancelar" e "Salvar Alterações" do próprio DetailPanel.
...
### Componente LinkTenantModal (UX-001-C04)

Modal para vincular um tenant (N5) a um nó N4. Acessível via menu de contexto "Vincular tenant" na árvore e via botão "+ Vincular Estabelecimento" no DetailPanel.
...
Dialog (480px, r:16)
├── DialogHeader
│   ├── "Vincular Estabelecimento" (20px 700 #111)
│   └── "Selecione um estabelecimento para vincular a '{orgUnitNome}'." (14px 400 #555)
├── SearchBar (debounce 300ms, placeholder "Buscar por nome ou código...")
├── Lista de resultados (max-h:320px, overflow-y auto)
│   ├── Item: "{codigo} — {nome}" + Botão "Vincular" (primary, sm)
│   ├── Loading: Skeleton (3 linhas)
│   ├── Empty: "Nenhum estabelecimento encontrado para o termo buscado."
│   └── All linked: "Todos os estabelecimentos já estão vinculados a esta unidade."
└── DialogFooter
...
| Evento | Ação |
|--------|------|
| Clique "Vincular" em item | `POST /api/v1/org-units/:orgUnitId/tenants` com `{ tenant_id }` |
| Sucesso (201) | Toast "Tenant '{codigo}' vinculado a '{orgUnitNome}'." + `onSuccess()` + fecha modal |
| Erro 409 | Toast "Este estabelecimento já está vinculado a esta unidade." |
| Erro genérico | Toast com mensagem do servidor |
| Busca | `GET /api/v1/tenants?status=ACTIVE&search=<termo>`, filtro client-side exclui `linkedTenantIds` |

**Hooks necessários:**

- `use-available-tenants.ts` — query `GET /api/v1/tenants?status=ACTIVE&search=<termo>` com filtro para excluir `linkedTenantIds`
- `use-link-tenant.ts` — mutation `POST /api/v1/org-units/:id/tenants` com invalidação de queries (`org-tree`, `org-unit-detail`)

**Estado no OrgTreePage:**
...
O handler `handleLinkTenant(id)` substitui o anterior `handleOpenEdit(id)` na prop `onLinkTenant` do `OrgTreeNode`.

### Toggle "Mostrar inativos" — Data Wiring (UX-001-C05)

O checkbox "Mostrar inativos" DEVE conectar-se ao backend para buscar dados reais de unidades inativas:

**API Client (`org-units.api.ts`):**
- `fetchOrgTree(includeInactive?: boolean, signal?)` — envia `?include_inactive=true` quando ativo

**Hook (`use-org-tree.ts`):**
- `useOrgTree(includeInactive = false)` — inclui `{ includeInactive }` na `queryKey` do React Query para cache separado por estado do toggle

**Página (`OrgTreePage.tsx`):**
- `useOrgTree(showInactive)` — passa o estado do checkbox diretamente ao hook

**Comportamento:**
- Toggle **desmarcado** (default): `GET /org-units/tree` sem query param → apenas unidades ativas
- Toggle **marcado**: `GET /org-units/tree?include_inactive=true` → unidades ativas + inativas
- Alternância rápida: React Query cancela request anterior via `signal`, sem race condition
- Filtragem client-side em `OrgTreeNode.visibleChildren` permanece como segunda camada (safety net)
