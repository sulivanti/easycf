/**
 * @contract FR-009, UX-MCP-002, SEC-010
 *
...
export function ExecutionDetailPanel({ detail, loading, onClose }: ExecutionDetailPanelProps) {
  ...
  return (
    <aside
      className="flex w-full flex-col gap-4 overflow-y-auto border-l bg-background p-4 lg:w-[40%]"
      role="complementary"
      aria-label="Detalhe da execucao"
    >
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Detalhe da Execucao</h3>
      ...
      {isEscalation && (
        <div className="rounded-md border border-destructive bg-destructive/10 p-3">
          <p className="text-sm font-bold text-destructive">TENTATIVA DE ESCALADA DE PRIVILEGIO</p>
      ...
      <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-sm">
        ...
        <dt className="font-medium text-muted-foreground">Acao</dt>
        <dd>{detail.action_id}</dd>

        <dt className="font-medium text-muted-foreground">Politica</dt>
        ...
        <dt className="font-medium text-muted-foreground">Duracao</dt>
        <dd>{detail.duration_ms != null ? `${detail.duration_ms}ms` : '\u2014'}</dd>

        <dt className="font-medium text-muted-foreground">Recebido em</dt>
      ...
      {detail.linked_movement_id && detail.status === 'CONTROLLED_PENDING' && (
        <div className="mt-2">
          <Button variant="outline" size="sm" asChild>
            <a href={`/aprovacoes/${detail.linked_movement_id}`}>Ver no inbox de aprovacoes</a>
          </Button>
        </div>
      )}
