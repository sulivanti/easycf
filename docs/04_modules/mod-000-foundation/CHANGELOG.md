...
| 1.24.0 | 2026-03-31 | merge-amendment | Merge FR-000-M05: campo `status` opcional no PUT /roles/:id (FR-007). UpdateRoleInput + updateRoleBody + frontend type recebem status ACTIVE/INACTIVE. Novo evento role.status_changed. Base FR-000 bumped para v0.12.0. Ref: UX-000-M04, 07-role-form-spec. |
| 1.23.0 | 2026-03-31 | merge-amendment | Merge UX-000-M04: design visual 07-RoleForm aplicado em UX-006. FormCard 720px, ScopeChips, Toggle status, InfoBox, BackLink. Dois modos (Editar/Criar). Base UX-000 bumped para v0.7.0. Ref: 07-role-form-spec, PEN-07-RoleForm-Edit, PEN-07-RoleForm-Create. |
| 1.22.0 | 2026-03-31 | cascade-amendment | Cascade de UX-000-M04: 2 amendments derivados criados — FR-000-M05 (campo `status` opcional no PUT /roles/:id), INT-000-M04 (schema OpenAPI `UpdateRoleRequest` + status). DATA-000 confirmado sem impacto (coluna já existe). SEC-000 confirmado INFORMACIONAL. |
| 1.21.0 | 2026-03-31 | create-amendment | Amendment UX-000-M04: design visual definitivo 07-RoleForm em UX-006. FormCard 720px, ScopeChips removíveis, Toggle status, InfoBox, BackLink. Dois modos (Editar/Criar) com rotas dedicadas. Ref: 07-role-form-spec, PEN-07-RoleForm-Edit, PEN-07-RoleForm-Create. |
| 1.20.2 | 2026-03-31 | merge-amendment | Merge FR-000-C10: rename scope `phase2-enable` → `phase2_enable` em FR-007. Base FR-000 bumped para v0.11.2. Ref: spec-fix-scope-hyphen-rename. |
| 1.20.1 | 2026-03-31 | create-amendment | Amendment FR-000-C10: rename scope `mcp:agent:phase2-enable` → `mcp:agent:phase2_enable` no seed + reverter CHECK constraint à regex canônica. Desfaz workaround DATA-000-C03. Ref: spec-fix-scope-hyphen-rename. |
