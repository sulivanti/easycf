...
// Read the Python file and extract the files dict by executing it as a simple converter
const pyContent = readFileSync('_forge_mod007.py', 'utf8');

// Extract file content blocks from the Python script
const base = "D:/Dev/EasyCodeFramework/docs/04_modules/mod-007-parametrizacao-contextual";

// We'll just parse the Python file and extract the string assignments
// Actually, let's just run node to write files directly

const AUTOMATION_HEADER = `> \u26a0\ufe0f **ARQUIVO GERIDO POR AUTOMA\u00c7\u00c3O.**
...
// mod.md
writeFile('mod.md', `${AUTOMATION_HEADER}
...
M\u00f3dulo com dom\u00ednio rico: motor de avalia\u00e7\u00e3o com 6 passos, ciclo de vida de rotinas (DRAFT\u2192PUBLISHED\u2192DEPRECATED), fork com c\u00f3pia deep, resolu\u00e7\u00e3o de conflito em duas camadas, 9 tabelas, 23 endpoints, integra\u00e7\u00e3o com MOD-006, e domain events.

### Justificativa (Score DOC-ESC-001 \u00a74.2: 5/6)

| Gatilho | Presente | Evid\u00eancia |
|---|---|---|
| Estado/workflow | **SIM** | Ciclo DRAFT\u2192PUBLISHED\u2192DEPRECATED; fork; vig\u00eancia |
| Compliance/auditoria | **SIM** | domain_events, change_reason, imutabilidade PUBLISHED |
| Concorr\u00eancia/consist\u00eancia | **SIM** | UNIQUE constraint, motor sem cache, safety net |
| Integra\u00e7\u00f5es externas cr\u00edticas | **N\u00c3O** | Sem providers externos |
| Multi-tenant | **SIM** | tenant_id, 7 scopes param:* |
| Regras cruzadas/reuso | **SIM** | Motor consumido por MOD-006 e frontend |
...
|---|---|
| MOD-000 (Foundation) | Auth, RBAC scopes, domain events, audit trail, tenant_id |
...
// CHANGELOG.md
writeFile('CHANGELOG.md', `${AUTOMATION_HEADER}
# CHANGELOG - MOD-007
...
// BR-007.md
writeFile('requirements/br/BR-007.md', `${AUTOMATION_HEADER}${BASELINE_TABLE}
...
- **Regra:** O sistema impede a cria\u00e7\u00e3o de regras de incid\u00eancia duplicadas para o mesmo par (framer_id, target_object_id). Ao detectar conflito, retorna 422.
...
- **Regra:** Se dois contextos conflitantes coexistirem em runtime, a regra mais restritiva vence. HIDE > SHOW, SET_REQUIRED > SET_OPTIONAL, dom\u00ednio menor prevalece. Campos n\u00e3o conflitantes s\u00e3o mesclados.
...
- **Regra:** Rotinas PUBLISHED n\u00e3o podem ser editadas (PATCH retorna 422). Fork \u00e9 obrigat\u00f3rio para modificar.
- **Impacto:** STATE, COMPLIANCE
- **estado_item:** DRAFT
...
## BR-004 \u2014 C\u00f3digo Imut\u00e1vel

- **Regra:** Campo \`codigo\` de enquadradores e rotinas \u00e9 imut\u00e1vel ap\u00f3s cria\u00e7\u00e3o.
- **Impacto:** STATE, COMPLIANCE
- **estado_item:** DRAFT
...
- **Regra:** Apenas rotinas PUBLISHED podem ser vinculadas a regras de incid\u00eancia.
- **Impacto:** STATE
- **estado_item:** DRAFT
...
## BR-006 \u2014 Motor Sem Cache

- **Regra:** Motor de avalia\u00e7\u00e3o executa ao vivo, sem cache Redis. Decis\u00e3o t\u00e9cnica 2026-03-15.
- **Impacto:** STATE, COMPLIANCE
- **estado_item:** DRAFT
...
- **Regra:** Fork exige change_reason obrigat\u00f3rio (min 10 chars). Copia itens e links.
- **Impacto:** COMPLIANCE
- **estado_item:** DRAFT
- **rastreia_para:** US-MOD-007, US-MOD-007-F02, DATA-007
...
## BR-008 \u2014 Publica\u00e7\u00e3o Exige Min 1 Item

- **Regra:** Rotina DRAFT s\u00f3 pode ser PUBLISHED com pelo menos 1 routine_item.
- **Impacto:** STATE
- **estado_item:** DRAFT
...
- **Regra:** Motor s\u00f3 considera enquadradores com vig\u00eancia ativa (valid_from <= now, valid_until null ou > now).
- **Impacto:** STATE
- **estado_item:** DRAFT
...
- **Regra:** Motor s\u00f3 persiste domain_events quando avalia\u00e7\u00e3o teve efeito.
- **Impacto:** STATE, COMPLIANCE
- **estado_item:** DRAFT
...
// FR-007.md
writeFile('requirements/fr/FR-007.md', `${AUTOMATION_HEADER}${BASELINE_TABLE}
...
- **Endpoints:** GET/POST/PATCH/DELETE /api/v1/admin/framers
- **Scope:** param:framer:read, param:framer:write, param:framer:delete
- **Done funcional:** Codigo UNIQUE uppercase imut\u00e1vel. Soft-delete. Vig\u00eancia.
- **estado_item:** DRAFT
...
- **Done funcional:** UNIQUE constraint bloqueia duplicatas com 422.
- **estado_item:** DRAFT
- **rastreia_para:** US-MOD-007-F01, BR-001, DATA-007

---

## FR-005 \u2014 CRUD de Rotinas com Versionamento
- **Endpoints:** GET/POST/PATCH /api/v1/admin/routines, POST .../publish, POST .../fork
- **Done funcional:** PATCH s\u00f3 DRAFT. Publish min 1 item. Fork copia tudo + change_reason.
- **Efeito colateral:** Sim (domain event)
- **estado_item:** DRAFT
...
- **Done funcional:** 7 tipos de item. Reordena\u00e7\u00e3o por campo ordem. S\u00f3 DRAFT.
- **estado_item:** DRAFT
...
- **Algoritmo:** 6 passos (find incidence, find routines, evaluate items, resolve conflicts, construct response, persist events)
...
- **Done funcional:** Sem cache. blocking_validations integra MOD-006. X-Correlation-ID propagado. condition_expr ignorado v1.
- **Efeito colateral:** Sim (domain_event)
- **estado_item:** DRAFT
...
- **Descri\u00e7\u00e3o:** Tabela de enquadradores, painel objetos-alvo, matriz de incid\u00eancia visual, simula\u00e7\u00e3o dry-run.
- **estado_item:** DRAFT
...
- **Descri\u00e7\u00e3o:** Split-view editor, formul\u00e1rio adaptativo, drag-and-drop, auto-save 600ms, publish modal, fork, readonly, preview, timeline.
- **estado_item:** DRAFT
- **rastreia_para:** US-MOD-007-F05, UX-007
`);

// pen-007-pendente.md (empty stub)
writeFile('requirements/pen-007-pendente.md', `${AUTOMATION_HEADER}${BASELINE_TABLE}
# PEN-007 \u2014 Pend\u00eancias da Parametriza\u00e7\u00e3o Contextual e Rotinas

> Nenhuma pend\u00eancia registrada no baseline.

- **estado_item:** DRAFT
- **owner:** Marcos Sulivan
- **data_ultima_revisao:** 2026-03-19
- **rastreia_para:** US-MOD-007
`);

console.log('\\nAll MOD-007 files created successfully!');
