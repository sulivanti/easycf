...
export const caseInstances = pgTable(
  ...
  (table) => [
    ...
    index('idx_case_instances_tenant_status').on(table.tenantId, table.status),

    // Filtro por ciclo
    index('idx_case_instances_cycle').on(table.cycleId),

    // Ordenação padrão da listagem (tenant + opened_at DESC)
    index('idx_case_instances_tenant_opened_at').on(table.tenantId, table.openedAt),
...
export const stageHistory = pgTable(
  ...
  (table) => [
    ...
    index('idx_stage_history_case').on(table.caseId, table.transitionedAt),
  ],
);
...
export const gateInstances = pgTable(
  'gate_instances',
  {
    ...
    checklistItems:
      jsonb('checklist_items').$type<Array<{ id: string; label: string; checked: boolean }>>(),
  },
  (table) => [
    // Status enum check
    check(
    ...
    uniqueIndex('gate_instances_case_gate_idx').on(table.caseId, table.gateId),
...
export const caseAssignments = pgTable(
  'case_assignments',
  {
    ...
    delegationId: uuid('delegation_id').references(() => accessDelegations.id, {
      onDelete: 'restrict',
    }),
  },
  (table) => [
    ...
    index('idx_case_assignments_valid_until')
      .on(table.validUntil)
      .where(sql`${table.isActive} = true AND ${table.validUntil} IS NOT NULL`),
  ],
);
