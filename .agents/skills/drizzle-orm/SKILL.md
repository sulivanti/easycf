...
> [!IMPORTANT]
> **Regra deste projeto:** Ao criar ou editar schemas Drizzle aqui, você **DEVE** ler e aplicar a skill **`validate-drizzle-schemas`** antes de finalizar qualquer arquivo de schema. Ela define regras mandatórias de arquitetura que esta skill não cobre: multi-tenant isolation (`tenant_id`), soft-delete (`deleted_at`), Zod integration e Event Sourcing (DATA-003).
