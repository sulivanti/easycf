...
```text
{{project_name}}/.env          ← único .env (não commitado)
{{project_name}}/.env.example  ← template obrigatório no repositório
```

Para configurar um novo ambiente basta:

```powershell
# Na raiz {{project_name}}/
cp .env.example .env
...
1. Confirmado `.env` único na raiz `{{project_name}}/`
